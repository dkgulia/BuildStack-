/**
 * Migration Script: Export PC Component Data from IGNIPC to Supabase
 *
 * This script:
 * 1. Connects to local IGNIPC PostgreSQL database
 * 2. Queries all PC component tables with resolved foreign keys
 * 3. Transforms data to new schema format (specs as JSONB)
 * 4. Outputs SQL INSERT statements for Supabase
 *
 * Usage:
 *   npx ts-node scripts/migrate-data.ts
 *
 * Output:
 *   Creates supabase/migrations/002_seed_ignipc_data.sql
 */
...
async function fetchCPUs(): Promise<ComponentRow[]> {
  const query = `
    SELECT
      c.cpu_id as id,
      c.name,
      ...
      c.tdp_watts,
      c.cache_mb,
      r.ram_type_name as ram_type,
      c.max_ram_speed_mhz,
      c.has_integrated_graphics,
      c.unlocked
    FROM pc_cpu c
  ...
  return result.rows.map((row) => ({
    ...
    specs: {
      ...
      integrated_graphics: row.has_integrated_graphics,
      unlocked: row.unlocked,
    },
  }));
}

async function fetchGPUs(): Promise<ComponentRow[]> {
  const query = `
    SELECT
      g.gpu_id as id,
      g.name,
  ...
  return result.rows.map((row) => ({
    ...
    specs: {
      ...
      base_clock_mhz: row.base_clock_mhz,
      boost_clock_mhz: row.boost_clock_mhz,
      cuda_cores: row.cuda_cores,
      stream_processors: row.stream_processors,
      length_mm: row.length_mm,
      slot_width: parseFloat(row.slot_width) || 2,
      tdp: row.tdp_watts,
      recommended_psu: row.recommended_psu_watts,
      power_connectors: row.power_connectors,
    },
  }));
}

async function fetchMotherboards(): Promise<ComponentRow[]> {
  const query = `
    SELECT
      ...
      s.socket_name as socket,
      m.chipset,
      f.form_factor_name as form_factor,
      r.ram_type_name as ram_type,
      m.ram_slots,
      m.max_ram_gb,
      m.m2_slots,
...
async function fetchRAM(): Promise<ComponentRow[]> {
  const query = `
    SELECT
      r.ram_id as id,
      r.name,
  ...
  return result.rows.map((row) => ({
    ...
    specs: {
      ...
      voltage: parseFloat(row.voltage) || null,
      has_rgb: row.has_rgb,
...
async function fetchStorage(): Promise<ComponentRow[]> {
  const query = `
    SELECT
      s.storage_id as id,
      s.name,
  ...
  return result.rows.map((row) => ({
    ...
    specs: {
      ...
      form_factor: row.form_factor,
      capacity_gb: row.capacity_gb,
      read_speed: row.read_speed_mbps,
      write_speed: row.write_speed_mbps,
      has_dram_cache: row.has_dram_cache,
      tbw: row.tbw,
    },
  }));
}

async function fetchPSUs(): Promise<ComponentRow[]> {
  const query = `
    SELECT
      p.psu_id as id,
      p.name,
      ...
      f.form_factor_name as form_factor,
      p.modular_type,
      p.cpu_power_connectors,
      p.gpu_power_connectors,
      p.sata_connectors
    FROM pc_psu p
  ...
  return result.rows.map((row) => ({
    ...
    specs: {
      ...
      form_factor: row.form_factor,
      modular: row.modular_type,
      cpu_connectors: row.cpu_power_connectors,
      gpu_connectors: row.gpu_power_connectors,
      sata_connectors: row.sata_connectors,
    },
  }));
}

async function fetchCases(): Promise<ComponentRow[]> {
  const query = `
    SELECT
      c.case_id as id,
      c.name,
  ...
  return result.rows.map((row) => ({
    ...
    specs: {
      ...
      form_factor: row.form_factors || [],
      max_gpu_length: row.max_gpu_length_mm,
      max_cooler_height: row.max_cooler_height_mm,
      max_psu_length: row.max_psu_length_mm,
      drive_bays_35: row.drive_bays_3_5,
      drive_bays_25: row.drive_bays_2_5,
      expansion_slots: row.expansion_slots,
      tempered_glass: row.has_tempered_glass,
      has_rgb: row.has_rgb,
    },
  }));
}

async function fetchCoolers(): Promise<ComponentRow[]> {
  const query = `
    SELECT
      c.cooler_id as id,
      c.name,
  ...
  return result.rows.map((row) => ({
    ...
    specs: {
      ...
      height_mm: row.height_mm,
      radiator_size: row.radiator_size_mm,
      fan_rpm_min: row.fan_rpm_min,
      fan_rpm_max: row.fan_rpm_max,
      noise_dba: row.noise_level_dba,
      tdp_rating: row.tdp_rating_watts,
      has_rgb: row.has_rgb,
...
function generateInsertSQL(components: ComponentRow[]): string {
  if (components.length === 0) return '';

  const values = components
    .map(
      (c) =>
        `(${escapeSQL(c.type)}, ${escapeSQL(c.name)}, ${escapeSQL(c.brand)}, ${escapeSQL(c.model)}, ${c.price}, ${escapeSQL(c.image_url)}, ${escapeSQL(c.specs)})`
    )
    .join(',\n  ');

  return `INSERT INTO components (type, name, brand, model, price, image_url, specs) VALUES\n  ${values};\n`;
}

async function main() {
  console.log('🚀 Starting IGNIPC → Supabase migration...\n');
  ...
  try {
    ...
    let sql = `-- IGNIPC PC Component Data Export
...
-- CPUs (${cpus.length})
-- ============================================================================
${generateInsertSQL(cpus)}

-- ============================================================================
-- GPUs (${gpus.length})
-- ============================================================================
${generateInsertSQL(gpus)}

-- ============================================================================
-- Motherboards (${motherboards.length})
-- ============================================================================
${generateInsertSQL(motherboards)}

-- ============================================================================
-- RAM (${rams.length})
-- ============================================================================
${generateInsertSQL(rams)}

-- ============================================================================
-- Storage (${storage.length})
-- ============================================================================
${generateInsertSQL(storage)}

-- ============================================================================
-- PSUs (${psus.length})
-- ============================================================================
${generateInsertSQL(psus)}

-- ============================================================================
-- Cases (${cases.length})
-- ============================================================================
${generateInsertSQL(cases)}

-- ============================================================================
-- Coolers (${coolers.length})
-- ============================================================================
${generateInsertSQL(coolers)}
`;
    ...
    console.log('   2. Run 001_initial_schema.sql in Supabase SQL Editor');
    console.log('   3. Run 002_seed_ignipc_data.sql in Supabase SQL Editor');
    console.log('   4. Update .env.local with Supabase URL and key');
